...
export class TableCfgJob2 {

    static TableName: string = "CfgJob2";

    private data: any;

    static cache: Map<number | string, TableCfgJob2> = new Map;

    static getConfig(id: number | string, error:boolean = true) {
        const hash = id;
        if (this.cache.has(hash)) return this.cache.get(hash);
        const table = JsonUtil.get("CfgJob2");
        const data = table[id];
        if (data) {
            const cfg = new TableCfgJob2(id, data);
            this.cache.set(hash, cfg);
            return cfg;
        }
        if (error) throw "TableCfgJob2 not found";
    }

    static list(): TableCfgJob2[] {
        let result = [];
        const all = JsonUtil.get("CfgJob2");
    ...
    static filter(cb: (cfg: TableCfgJob2, key?: number | string) => boolean | void): TableCfgJob2[] {
        let result = [];
        const all = JsonUtil.get("CfgJob2");
        const keys = Object.keys(all);
        for (let i = 0; i < keys.length; i++) {
    ...
    static forEach(cb: (cfg: TableCfgJob2, key?: number | string) => boolean | void) {
        const all = JsonUtil.get("CfgJob2");
        const keys = Object.keys(all);
        for (let i = 0; i < keys.length; i++) {
            const id = keys[i];
