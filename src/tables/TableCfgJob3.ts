...
export class TableCfgJob3 {

    static TableName: string = "CfgJob3";

    private data: any;

    static cache: Map<number | string, TableCfgJob3> = new Map;

    static getConfig(id: number | string, error:boolean = true) {
        const hash = id;
        if (this.cache.has(hash)) return this.cache.get(hash);
        const table = JsonUtil.get("CfgJob3");
        const data = table[id];
        if (data) {
            const cfg = new TableCfgJob3(id, data);
            this.cache.set(hash, cfg);
            return cfg;
        }
        if (error) throw "TableCfgJob3 not found";
    }

    static list(): TableCfgJob3[] {
        let result = [];
        const all = JsonUtil.get("CfgJob3");
    ...
    static filter(cb: (cfg: TableCfgJob3, key?: number | string) => boolean | void): TableCfgJob3[] {
        let result = [];
        const all = JsonUtil.get("CfgJob3");
        const keys = Object.keys(all);
        for (let i = 0; i < keys.length; i++) {
    ...
    static forEach(cb: (cfg: TableCfgJob3, key?: number | string) => boolean | void) {
        const all = JsonUtil.get("CfgJob3");
        const keys = Object.keys(all);
        for (let i = 0; i < keys.length; i++) {
            const id = keys[i];
